...
// Fiji — Stub pack. Exists from day 1 so an entity can be modeled
...
export default defineJurisdiction<Config>({
  ...
  validateEntityType(type) {
    return { ok: SUPPORTED_TYPES.includes(type), reason: SUPPORTED_TYPES.includes(type) ? undefined : 'Fiji stub pack does not enumerate this type yet' };
  },

  async searchName(ctx, name) {
    ctx.log('fj · name-check (stub) ' + name);
  ...
  async generatePlan(ctx, entity) {
    ctx.log('fj · plan (stub) ' + entity.slug);
    return {
      planId: 'p_' + Date.now(),
      entityId: entity.entityId,
      selectedPackId: 'entity-fj',
      selectedEntityType: entity.entityType,
      supportLevel: 'stub',
      requiredInputs: ['local-counsel', 'registered-office', 'director-ids'],
      manualSteps: ['research Fiji incorporation process', 'prepare documents with local counsel', 'file directly with local registry'],
  ...
  async generateDocs(ctx, entity) {
    ctx.log('fj · docs (stub) ' + entity.slug);
    return {
  ...
  async submitOrHandoff(ctx, entity, _bundle, mode) {
    ctx.log('fj · handoff (stub) ' + entity.slug + ' mode=' + mode);
  ...
  async createComplianceTasks(ctx, entity) {
    ctx.log('fj · compliance (stub) ' + entity.slug);
    return [
      { taskId: 't_' + Date.now(), entityId: entity.entityId, taskType: 'manual-review', title: 'Review Fiji ongoing compliance obligations with local counsel', description: 'Stub pack — no automated compliance calendar yet.', status: 'open', recurrence: 'once' },
    ];
  },

  async syncStatus(ctx, entity) {
    ctx.log('fj · status (stub) ' + entity.slug);
    return { status: entity.status };
  },

  setup: manualSetup({
    label: "Fiji (RORBA)",
    vendorDocUrl: "https://www.fiji.gov.fj/",
    steps: [
      "Fiji (RORBA) pack supplies entity-formation rules + filing templates.",
