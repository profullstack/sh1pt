...
// Tanzania — Stub pack. Exists from day 1 so an entity can be modeled
...
export default defineJurisdiction<Config>({
  ...
  validateEntityType(type) {
    return { ok: SUPPORTED_TYPES.includes(type), reason: SUPPORTED_TYPES.includes(type) ? undefined : 'Tanzania stub pack does not enumerate this type yet' };
  },

  async searchName(ctx, name) {
    ctx.log('tz · name-check (stub) ' + name);
  ...
  async generatePlan(ctx, entity) {
    ctx.log('tz · plan (stub) ' + entity.slug);
    return {
      planId: 'p_' + Date.now(),
      entityId: entity.entityId,
      selectedPackId: 'entity-tz',
      selectedEntityType: entity.entityType,
      supportLevel: 'stub',
      requiredInputs: ['local-counsel', 'registered-office', 'director-ids'],
      manualSteps: ['research Tanzania incorporation process', 'prepare documents with local counsel', 'file directly with local registry'],
  ...
  async generateDocs(ctx, entity) {
    ctx.log('tz · docs (stub) ' + entity.slug);
    return {
  ...
  async submitOrHandoff(ctx, entity, _bundle, mode) {
    ctx.log('tz · handoff (stub) ' + entity.slug + ' mode=' + mode);
  ...
  async createComplianceTasks(ctx, entity) {
    ctx.log('tz · compliance (stub) ' + entity.slug);
    return [
      { taskId: 't_' + Date.now(), entityId: entity.entityId, taskType: 'manual-review', title: 'Review Tanzania ongoing compliance obligations with local counsel', description: 'Stub pack — no automated compliance calendar yet.', status: 'open', recurrence: 'once' },
    ];
  },

  async syncStatus(ctx, entity) {
    ctx.log('tz · status (stub) ' + entity.slug);
    return { status: entity.status };
  },

  setup: manualSetup({
    label: "Tanzania (BRELA)",
    vendorDocUrl: "https://www.brela.go.tz/",
    steps: [
      "Tanzania (BRELA) pack supplies entity-formation rules + filing templates.",
