...
const DEFAULT_BASE = 'https://api.inceptionlabs.ai';
...
export default defineAi<Config>({
  ...
  async generate(ctx, prompt, opts, config) {
    ...
    if (!apiKey) throw new Error('INCEPTION_API_KEY not in vault');
    const model = opts.model ?? 'mercury';
    ...
    if (!res.ok) throw new Error(`Inception ${res.status}: ${redact(await res.text(), apiKey).slice(0, 200)}`);
