...
// Trinidad and Tobago — Stub pack. Exists from day 1 so an entity can be modeled
...
export default defineJurisdiction<Config>({
  ...
  validateEntityType(type) {
    return { ok: SUPPORTED_TYPES.includes(type), reason: SUPPORTED_TYPES.includes(type) ? undefined : 'Trinidad and Tobago stub pack does not enumerate this type yet' };
  },

  async searchName(ctx, name) {
    ctx.log('tt · name-check (stub) ' + name);
  ...
  async generatePlan(ctx, entity) {
    ctx.log('tt · plan (stub) ' + entity.slug);
    return {
      planId: 'p_' + Date.now(),
      entityId: entity.entityId,
      selectedPackId: 'entity-tt',
      selectedEntityType: entity.entityType,
      supportLevel: 'stub',
      requiredInputs: ['local-counsel', 'registered-office', 'director-ids'],
      manualSteps: ['research Trinidad and Tobago incorporation process', 'prepare documents with local counsel', 'file directly with local registry'],
  ...
  async generateDocs(ctx, entity) {
    ctx.log('tt · docs (stub) ' + entity.slug);
    return {
  ...
  async submitOrHandoff(ctx, entity, _bundle, mode) {
    ctx.log('tt · handoff (stub) ' + entity.slug + ' mode=' + mode);
  ...
  async createComplianceTasks(ctx, entity) {
    ctx.log('tt · compliance (stub) ' + entity.slug);
    return [
      { taskId: 't_' + Date.now(), entityId: entity.entityId, taskType: 'manual-review', title: 'Review Trinidad and Tobago ongoing compliance obligations with local counsel', description: 'Stub pack — no automated compliance calendar yet.', status: 'open', recurrence: 'once' },
    ];
  },

  async syncStatus(ctx, entity) {
    ctx.log('tt · status (stub) ' + entity.slug);
    return { status: entity.status };
  },

  setup: manualSetup({
    label: "Trinidad & Tobago",
    vendorDocUrl: "https://www.legalaffairs.gov.tt/",
    steps: [
      "Trinidad & Tobago pack supplies entity-formation rules + filing templates.",
