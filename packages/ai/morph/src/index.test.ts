...
const ctx = (secrets: Record<string, string> = { MORPH_API_KEY: 'test-key' }, dryRun = false) => ({
  secret: (key: string) => secrets[key],
  log: () => {},
  dryRun,
});

describe('Morph OpenAI-compatible generation', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it('short-circuits dry-run before network calls', async () => {
    ...
    const result = await adapter.generate(ctx({ MORPH_API_KEY: 'test-key' }, true), 'hello', {}, {});

    expect(result).toEqual({ text: '[dry-run]', model: 'morph-v2' });
  ...
  it('posts chat completions requests and maps usage tokens', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        choices: [{ message: { content: 'hi from morph' } }],
        model: 'morph-v2',
  ...
  it('normalizes configured base URLs with trailing slashes', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ choices: [{ message: { content: 'ok' } }], model: 'morph-v2' }),
    });
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('includes status and redacted response body excerpt on errors', async () => {
    ...
    let error: unknown;
    try {
      await adapter.generate(ctx({ MORPH_API_KEY: apiKey }), 'hello', {}, {});
    } catch (exc) {
      error = exc;
    }
    expect(error).toBeInstanceOf(Error);
    expect((error as Error).message).toContain('Morph 429:');
  ...
  it('throws when the API key is missing from the vault', async () => {
    await expect(adapter.generate(ctx({}), 'hello', {}, {})).rejects.toThrow('MORPH_API_KEY not in vault');
  });
});
